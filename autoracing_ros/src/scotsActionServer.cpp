	...
	public:
		...
		*/
		void publishDrive(double speed,double steering_angle){
			...
			drive_msg.drive.steering_angle_velocity = steering_angle;
		...
		*/
		void simulatePath(const scots::StaticController &controller, const autoracing_msgs::Target &tr) {
			...
			while(ros::ok()) {
				...
				std::vector<input_type> control_inputs = controller.peek_control<state_type, input_type>(robot_state);
				// std::sort(control_inputs.begin(), control_inputs.end(), comparing);
				ROS_INFO_STREAM("Controller is fine");
				vehicle_post(robot_state, control_inputs[0]);

				ROS_INFO_STREAM("Not vehicle post as well");
				...
				trajectory.poses.push_back(trajectory_poses);
				ROS_INFO_STREAM("Trajectory should have started");
		...
		void processGoal(const autoracing_msgs::AutoRacingGoalConstPtr &goal) {
			...
			*/
			...
			auto radius_post = [](state_type &r, const state_type &, const input_type &u) {
				...
				double tan_alpha = std::tan(max_steering_angle) * dr;
			  	r[0] = r[0] + std::abs(u[0])*(tan_alpha * r[2]+ r[3]*(1  + std::pow(tan_alpha,2))) * tau + w[0];
			  	r[1] = r[1] + std::abs(u[0])*(tan_alpha * r[2]+ r[3]*(1  + std::pow(tan_alpha,2))) * tau + w[1];
				r[2] = r[2] + std::abs(u[0]) * (1 + std::pow(std::tan(max_steering_angle),2)) / (lr+lf) * tau * r[3] + w[2];
			};			
			...
			*/
			...
			double wb = 0.3302;
			double max_speed = 7, max_steering_angle = 0.41, max_accel = 7.51, max_dec = -8.26, max_steering_vel = 3.2, max_yaw = 3.5;
